...
SerialBackendPosix::~SerialBackendPosix() {
...
Result SerialBackendPosix::OpenErrorFromErrno(int err, const std::string& path) {
    switch (err) {
        case ENOENT:
        case ENOTDIR:
        case ENXIO:
...
Result SerialBackendPosix::ApplyTermios(const ConnectTarget& config) {
    termios t{};
...
Result SerialBackendPosix::open(const ConnectTarget& config) {
    std::lock_guard<std::mutex> lk(mu_);
    if (fd_ >= 0) {
...
Result SerialBackendPosix::close() {
    std::lock_guard<std::mutex> lk(mu_);
    if (fd_ < 0) {
...
Result SerialBackendPosix::write(const Bytes& data) {
    std::lock_guard<std::mutex> lk(mu_);
    if (fd_ < 0) {
        return Result::Err(ERR_DEVICE_NOT_OPEN, "设备未打开");
    }
...
bool SerialBackendPosix::isOpen() const {
    std::lock_guard<std::mutex> lk(mu_);
    return fd_ >= 0;
}

domain::ProbeResult SerialBackendPosix::probe(const ConnectTarget& params,
...
int64_t SerialBackendPosix::nativeHandle() const {
    std::lock_guard<std::mutex> lk(mu_);
    return static_cast<int64_t>(fd_);
}

bool SerialBackendPosix::isBusy(const std::string& port) const {
